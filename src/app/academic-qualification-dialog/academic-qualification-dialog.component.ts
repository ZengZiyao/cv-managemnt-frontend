...
export class AcademicQualificationDialogComponent implements OnInit {
  ...
  constructor(
    private dialogRef: MatDialogRef<AcademicQualificationDialogComponent>,
    private fb: FormBuilder,
    @Inject(MAT_DIALOG_DATA) data,
    private academicQualificationService: AcademicQualificationService
  ) {
    this.academicQualificationCopy =
      data == null ? new AcademicQualification() : data;
  ...
  createForm() {
    this.academicQualificationForm = this.fb.group({
      university: [
        this.academicQualificationCopy.university,
        [Validators.required],
      ],
  ...
  onSubmit() {
    if (this.academicQualificationForm.valid) {
      ...
      if (this.academicQualificationCopy.id === undefined) {
        this.academicQualificationService
          .addAcademicQualification(data)
          .subscribe(() => {
            this.dialogRef.close();
          });
      } else {
        this.academicQualificationService
          .updateAcademicQualification(this.academicQualificationCopy.id, data)
          .subscribe(() => {
            this.dialogRef.close();
          });
      }
