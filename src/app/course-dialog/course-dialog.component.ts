...
export class CourseDialogComponent implements OnInit {
  ...
  constructor(    private dialogRef: MatDialogRef<CourseDialogComponent>,
    ...
    this.currentWorking = this.courseCopy.endYear == null;
  ...
  onSubmit() {
    let data: Course = this.courseForm.value;
    ...
    if (data.courseType == "OTHERS") {
      data.courseType = this.otherType;
    }

    if (this.courseCopy.id === undefined) {
      this.courseService.addCourse(data).subscribe(() => this.dialogRef.close());
    } else {
      this.courseService.updateCourse(this.courseCopy.id, data).subscribe(() => this.dialogRef.close());
    }
  }
